...
(function (root, factory) {
  ...
  var METHODS = {
    ...
    // ---- 拓展档：教材里标 ★ 的那几道，新方法也跟着来 ----
    // 这三个方法都是"题面里少了一步现成的条件"，所以先要自己把条件理出来。
    'M-TWO-CLASS': {
      id: 'M-TWO-CLASS',
      name: '分两类算',
      tip: '题目里有两类人（或两种价钱），就分两摊各算各的，最后再加起来。先把"谁按哪个价"标清楚，别看见两个数就一起乘。',
      steps: ['标出两类人各有多少', '标出各自是什么价', '各算一摊', '两摊加起来']
    },
    'M-SAME-DIST': {
      id: 'M-SAME-DIST',
      name: '抓住不变的量',
      tip: '去和回走的是同一段路 —— 路程不变，变的是速度。先用去时的速度×时间把这段路求出来，再用它算回来。',
      steps: ['找出那个不变的量（同一段路）', '用已知的一头把它算出来', '再用它去求问的那一头']
    },
    'M-BIGGEST': {
      id: 'M-BIGGEST',
      name: '大数占高位，两数要挨着',
      tip: '要乘积最大：大的数字往高位放（百位、十位）；两个数还要挨得近 —— 一个特别大、一个特别小，乘出来反而小。',
      steps: ['把数字从大到小排', '最大的放百位、接着放两位数的十位', '让两个数挨得近（大的那个别太孤单）', '不放心就把几个候选都乘一遍比一比']
    },
    // ---- 第六单元 长方形、正方形的面积 ----
    'M-AREA-RECT': {
      id: 'M-AREA-RECT',
  ...
  /* ============================ 难度分级 ============================
   *
   * 四档的界线不是"数字更大、字数更多"，而是**要跨几步、要不要转化**：
   *   基础 —— 和课本例题同构，一步就能出答案（教材的"试一试"、教案的"基础问题"）
   *   巩固 —— 绕一个弯：反过来求、或者换个样子（教材的"课堂活动"、教案的"提升问题"、
   *            第五单元教案里那条"逆向推导除法关系式"）
   *   挑战 —— 要把两个关系接起来，或者要说理（归一/归总两步、积的变化规律、
   *            近似数反推最大最小、组合图形割补）
   *   拓展 —— 条件要自己整理，或者要两步以上串起来；就是教材里标了 ★ 的那几道
   *            （P53 思考题"用五个数字组成乘积最大的算式"、P53 第 12 题"老师和学生
   *            两种票价"、P52 例 1 表格里两个乘数各乘一个不同的数）
   *
   * 第四档是后来加的：孩子练了 6 场、59 题全对，原话是"太简单"。
   * 而那时候最深的一道题才 0.68 —— 天花板就在那儿，练得再好也只能见到它。
   * 加这一档不是把题改难，是把教材自己留在那儿的那层补进来。
   *
   * min / max 是这一档的**区间**（tierOf 只拿 max 判，min 是给"出题难度抬到哪一档"
   * 用的，见 engine.js 的 targetDifficulty；那份区间不该再抄一份数字）。
   *
   ...
   * 把挑战题摆出来，被打击的是同一个孩子。所以高档题做成"解锁"而不是"随机"——
   * 基础没稳，第三、四档根本不出；基础稳了，它们自己冒出来。
   */
  var TIERS = [
    {
      level: 1, key: 'BASE', name: '基础', min: 0, max: 0.42, unlock: 0,
      tip: '和课本例题一样，一步一步就能算出来'
    },
    {
      level: 2, key: 'SOLID', name: '巩固', min: 0.42, max: 0.56, unlock: 0.45,
      tip: '要绕个弯：反过来想，或者换个样子认它'
    },
    {
      level: 3, key: 'CHALLENGE', name: '挑战', min: 0.56, max: 0.70, unlock: 0.70,
      tip: '要把两件事接在一起，或者要把道理说清楚'
    },
    {
      level: 4, key: 'EXTEND', name: '拓展', min: 0.70, max: 1.01, unlock: 0.85,
      tip: '要先自己把条件理清楚，或者把三步以上接起来'
    }
  ];
